...
export class PetService implements OnDestroy {
  ...
  readonly pets = computed(() => [...DEMO_PETS, ...this.records()]);
  ...
  private async synchronize(): Promise<void> {
    ...
    for (const pet of remote) {
      if (!DEMO_PETS.some((demo) => demo.id === pet.id) && !merged.get(pet.id)?.pending) {
    ...
    while (!this.destroyed && this.connectivity.isOnline()) {
      ...
      this.writeRecords(this.records().map((item) => item.id === pet.id ? { ...item, pending: false } : item));
    }
  }

  private async fetchJson(options?: RequestInit): Promise<unknown> {
    const controller = new AbortController();
    this.request = controller;
    const timeout = setTimeout(() => controller.abort(), 10_000);
    try {
      const response = await fetch(environment.petsApiUrl, { ...options, signal: controller.signal });
