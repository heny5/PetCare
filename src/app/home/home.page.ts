...
export class HomePage implements OnInit, OnDestroy {
  ...
  alertas: AlertaCollar[] = [
    { id: 1, tipo: 'critica', titulo: 'Temperatura elevada', detalle: 'Se detectaron 39.4 °C durante 5 minutos.', hora: 'Hoy · 2:18 p. m.', revisada: false },
    { id: 2, tipo: 'advertencia', titulo: 'Batería baja', detalle: 'El collar tiene 18% de batería disponible.', hora: 'Hoy · 1:42 p. m.', revisada: false },
  ...
  conectar(): void {
    ...
    this.mensaje = 'Collar PetCare vinculado correctamente.';
  }
  ...
  buscarDispositivos(): void {
    ...
    setTimeout(() => {
      ...
      this.mensaje = 'Se encontró 1 dispositivo compatible.';
    }, 1200);
  }

  desconectar(): void {
    this.dispositivoConectado = false;
    this.mensaje = 'Collar PetCare desconectado.';
    this.agregarAlertaDesconexion();
  }

  simularNfc(): void {
    this.lecturaNfc = true;
    this.mensaje = 'Etiqueta detectada: PET-LUNA-001';
  }

  compartir(): void {
    this.mensaje = 'Expediente preparado para compartir de forma segura.';
  }
  ...
  alternarMascotaPerdida(): void {
    this.mascotaPerdida = !this.mascotaPerdida;
    this.hallazgoRegistrado = false;
    this.fechaAlertaPerdida = this.mascotaPerdida
      ? new Date().toLocaleString('es-DO', { dateStyle: 'medium', timeStyle: 'short' })
      : '';
    this.mensaje = this.mascotaPerdida
  ...
  registrarHallazgo(): void {
    ...
    this.mensaje = 'Hallazgo registrado en Santiago, República Dominicana.';
    this.guardarEstadoLocal();
  }

  revisarAlerta(id: number): void {
    this.alertas = this.alertas.map((alerta) => alerta.id === id ? { ...alerta, revisada: true } : alerta);
    this.mensaje = 'Alerta marcada como revisada y añadida al historial de Luna.';
    this.guardarEstadoLocal();
  }
  ...
  crearAlertaPrueba(): void {
    const nueva: AlertaCollar = {
      ...
      detalle: 'No se recibe señal del collar PetCare PC-01.',
      hora: 'Ahora',
      revisada: false,
    };
    this.alertas = [nueva, ...this.alertas];
    this.mensaje = 'Alerta de demostración generada.';
    this.guardarEstadoLocal();
  }

  get alertasPendientes(): number {
    return this.alertas.filter((alerta) => !alerta.revisada).length;
  }

  private agregarAlertaDesconexion(): void {
    if (this.alertas.some((alerta) => alerta.titulo === 'Collar desconectado' && !alerta.revisada)) return;
  ...
  private cargarEstadoLocal(): void {
    try {
      ...
      const alertasGuardadas = localStorage.getItem('petcare-alertas');
      if (alertasGuardadas) this.alertas = JSON.parse(alertasGuardadas);
    } catch {
      // Si el almacenamiento local está dañado, se mantienen los valores iniciales.
    }
  }

  private guardarEstadoLocal(): void {
    localStorage.setItem('petcare-mascota-perdida', JSON.stringify(this.mascotaPerdida));
    localStorage.setItem('petcare-fecha-perdida', this.fechaAlertaPerdida);
    localStorage.setItem('petcare-hallazgo', JSON.stringify(this.hallazgoRegistrado));
    localStorage.setItem('petcare-alertas', JSON.stringify(this.alertas));
  }
}
